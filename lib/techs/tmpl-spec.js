    ...
    .builder(function (referencesFilename) {
        ...
        return readAsset.then(function (asset) {
            return template(asset, {
                ...
                paths: {
                    references: ['.', path.relative(nodeDirname, referencesFilename).replace(/\\/g, '\/')].join('/'),
                    'html-differ': ['.', path.relative(nodeDirname, htmlDifferFilename).replace(/\\/g, '\/')].join('/'),
                    'js-beautify': ['.', path.relative(nodeDirname, jsBeautifyFilename).replace(/\\/g, '\/')].join('/')
                },
                saveHtml: saveHtml
            });
        });
